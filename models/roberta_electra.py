...
class RobertaElectra(RobertaPreTrainedModel):
    ...
    def __init__(self, pretrained_model_name_or_path, config):
        ...
        self.electra = AutoModel.from_pretrained(
            "tunib/electra-ko-base", config=electra_config
        )
        self.electra_hidden_dim = electra_config.hidden_size

        self.qa_outputs = nn.Linear(
            in_features=self.roberta_hidden_dim + self.electra_hidden_dim,
    ...
    def forward(
        ...
        concat_logit = torch.cat(
            (roberta_sequence_output, electra_sequence_output), dim=-1
        )
