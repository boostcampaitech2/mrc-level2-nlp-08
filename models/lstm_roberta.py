...
class LSTMRobertaForQuestionAnswering(RobertaPreTrainedModel):
    ...
    def __init__(self, pretrained_model_name_or_path, config):
        ...
        kernel_size = 128
        size_after_conv = lambda input_length, kernel_size: input_length - kernel_size + 1

        self.conv1d = nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=kernel_size)
        self.linear = nn.Linear(
            in_features=size_after_conv(input_length, kernel_size), out_features=input_length
        )
        self.lstm = nn.LSTM(
    ...
    def forward(
        ...
        conv1d_output = self.conv1d(sequence_output.transpose(1, 2))
        # print(f'{conv1d_output.shape=}')
        linear_output = self.linear(conv1d_output)
        # print(f"{linear_output.shape=}")
        lstm_output, (h_n, c_n) = self.lstm(linear_output.transpose(1, 2))
