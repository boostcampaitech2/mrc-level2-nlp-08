...
def create_processed_datasets(data_path="/opt/ml/data/"):
    ...
    train_dataset = load_from_disk("/opt/ml/train_with_origin_gt_add_top_k_passage/not_include_answer_passage_train_es_top_4")
    ...
    Dataset.from_pandas(new_train_data, features=train_features).save_to_disk(
        "/opt/ml/data/new_train_dataset/toptrain"
    )
