...
class MyTrainingArguments:
    ...
    eval_steps: int = field(default=500, metadata={
    ...
    xpu_backend: str = field(
        default=None,
        metadata={"help": "The backend to be used for distributed training on Intel XPU.",
                  "choices": ["mpi", "ccl"]},
    )
    ...
    hub_strategy: HubStrategy = field(
        default="every_save",
        metadata={
            "help": "The hub strategy to use when `--push_to_hub` is activated."},
    )
    ...
    push_to_hub_token: str = field(default=None, metadata={
                                   "help": "The token to use to push to the Model Hub."})
    ...
    def __str__(self):
        ...
        self_as_dict = {k: f"<{k.upper()}>" if k.endswith(
            "_token") else v for k, v in self_as_dict.items()}
    ...
    def get_warmup_steps(self, num_training_steps: int):
        ...
        warmup_steps = (
            self.warmup_steps if self.warmup_steps > 0 else math.ceil(
                num_training_steps * self.warmup_ratio)
        )
        return warmup_steps
