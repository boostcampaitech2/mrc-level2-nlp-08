    ...
    def evaluation_loop(
            ...
            for step, inputs in enumerate(dataloader):
                ...
                if scores is not None:
                    ...
                    #print(f"scores after nested gather: {scores}")
                    scores_host = scores if scores_host is None else torch.cat((scores_host, scores), dim=0)
                    #print(f"scores after torch.cat: {scores_host}")
                ...
                if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                    ...
                    if scores_host is not None:
                        ...
                        #print(f"scores after nested_numpify scores_host: {scores}")
            ...
            if scores_host is not None:
                scores = nested_numpify(scores_host)
                #print(f"no accum scores after nested_numpify scores_host: {scores}")
                all_scores = scores if all_scores is None else np.concatenate((all_scores, scores), axis=0)
                #print(f"no accum all scores: {all_scores}")
            ...
            if all_scores is not None:
                ...
                #print(f"if all_scores is not None: {all_scores}")

            # Metrics!
            if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
                metrics = self.compute_metrics(EvalPrediction(predictions=all_preds, label_ids=all_labels, scores=all_scores))
            else:
                metrics = {}
