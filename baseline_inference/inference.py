...
def run_dense_retrival(text_data, top_k):
    wiki_id_ctx = get_pickle("/opt/ml/data/wiki_id_context_pair.bin")
    wiki_ctx_id = get_pickle("/opt/ml/data/wiki_context_id_pair.bin")
    test_data = get_pickle("/opt/ml/data/hybrid_retrieval_300.bin")
    ...
    for i in range(len(text_data["validation"]["id"])):
        ...
        for j in range(top_k):
            texts.append(wiki_id_ctx[test_data[question_texts[i]][j]])
        total.append(" ".join(texts))

    # " ".join(test_data[question_texts[i]][:top_k])
    df = pd.DataFrame(
        data={
